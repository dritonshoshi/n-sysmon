...
(function () {
    var nsysmon = angular.module('nsysmon', ['ngRoute'], function() {
    });
    ...
    nsysmon.service('config', ['$location', 'configRaw', function($location, configRaw) {
        var byPageId = {};
    ...
    nsysmon.constant('formatNumber', function(number, numFracDigits) {
        if(!number && number !== 0) {
            return '';
        }
    ...
    nsysmon.constant('escapeHtml', escapeHtml);


    nsysmon.constant('startsWith', function (s, prefix) {
        s = s || '';
        prefix = prefix || '';
    ...
    nsysmon.service('Modal', function() {
